...
from ..dataset.dataset import DatasetHMR, DatasetHMRPart
...
class HMRPartTrainer(HMRTrainer):
    ...
    def train_dataset(self):
        options = self.hparams.DATASET
        dataset_names = options.DATASETS_AND_RATIOS.split('_')
        dataset_list = [DatasetHMRPart(options, ds) for ds in dataset_names]
        parts_count = options['PARTS_COUNT']
        if parts_count > 0:
            part_idx = options['PART_IDX']
            for dataset in dataset_list:
                elems_count = len(dataset)
    ...
    def val_dataset(self):
        datasets_str = self.hparams.DATASET.VAL_DS
        datasets = [] if datasets_str is None else self.hparams.DATASET.VAL_DS.split('_')
        logger.info(f'Validation datasets are: {datasets}')
        val_datasets = []
        for dataset_name in datasets:
            dataset = DatasetHMRPart(
                options=self.hparams.DATASET,
                dataset=dataset_name,
                is_train=False,)
            parts_count = self.hparams.DATASET['PARTS_COUNT']
            if parts_count > 0:
                part_idx = self.hparams.DATASET['PART_IDX']
                elems_count = len(dataset)
                part_size = elems_count // parts_count
                if part_size > 0:
                    dataset.set_used_idxs(np.arange(start=part_idx * part_size, stop=(part_idx + 1) * part_size))
